...
const insertTx = async (fields={}) => {
  ...
  return scalar(`INSERT INTO transactions(${cols.join(',')}) VALUES (${cols.map((_,i)=>'$'+(i+1)).join(',')}) RETURNING id`,Object.values(data));
};
try {
  ...
  await db.exec(await sqlFile('../supabase/migrations/20260904190100_fix_financial_cash_series.sql'));
  await db.exec(`INSERT INTO auth.users(id) VALUES ('${uid}'),('${other}'),('${viewer}');
    INSERT INTO user_roles(user_id,role,owner_id) VALUES ('${uid}','admin',null),('${other}','admin',null),('${viewer}','viewer','${uid}');`);
  const account = await scalar("INSERT INTO accounts(user_id,name,initial_balance) VALUES ($1,'Bank',1000) RETURNING id",[uid]);
  const wallet = await scalar("INSERT INTO accounts(user_id,name,type,initial_balance) VALUES ($1,'Cash','cash',50) RETURNING id",[uid]);
  const foreignAccount = await scalar("INSERT INTO accounts(user_id,name) VALUES ($1,'Other') RETURNING id",[other]);
  const foreignCard = await scalar("INSERT INTO credit_cards(user_id,name) VALUES ($1,'Other card') RETURNING id",[other]);
  const card = await scalar("INSERT INTO credit_cards(user_id,name,total_limit,closing_day,due_day) VALUES ($1,'Card',4000,5,12) RETURNING id",[uid]);
  ...
  await check('transfer moves two balances without changing cash or expenses',async()=>{
    transfer=await insertTx({type:'transfer',amount:100,account_id:account,destination_account_id:wallet});
    assert.deepEqual((await balances()).map(b=>({...b,balance:Number(b.balance)})),[{name:'Bank',balance:900},{name:'Cash',balance:150}]);
    assert.equal(Number(await scalar("SELECT get_financial_overview()->>'saldo_disponivel'")),1050);
    assert.equal(Number(await scalar("SELECT despesas FROM get_monthly_financial_summary('2026-09-01','2026-09-30')")),0);
  });
  await check('card purchase creates invoice without debiting account',async()=>{
    ...
    bill=await scalar('SELECT bill_id FROM credit_card_bill_items WHERE transaction_id=$1',[purchase]);
    assert.equal(await scalar('SELECT due_date FROM credit_card_bills WHERE id=$1',[bill]),'2026-09-12');
  ...
  await check('editing and deleting card purchases reverse invoice and limit',async()=>{
    ...
    assert.equal(Number(await limit()),80);
    assert.equal(Number(await scalar('SELECT amount FROM credit_card_bills WHERE id=$1',[bill])),80);
  });
  await check('closing date, next cycle, short months and year rollover',async()=>{
    for(const [date,close,due,expected] of [
      ['2026-09-05',5,12,'2026-09-12'],['2026-09-06',5,12,'2026-10-12'],
      ['2026-12-30',25,5,'2027-02-05'],['2028-02-10',15,31,'2028-02-29'],
      ['2026-02-10',15,31,'2026-02-28'],['2026-04-30',31,5,'2026-05-05']]) {
  ...
  await check('installments conserve cents and reserve the full outstanding credit',async()=>{
    const id=await insertTx({credit_card_id:card,amount:100,installment_count:3,occurred_at:'2026-09-06'});
    const items=await query('SELECT installment_number,amount FROM credit_card_bill_items WHERE transaction_id=$1 ORDER BY installment_number',[id]);
    assert.deepEqual(items.map(i=>Number(i.amount)),[33.34,33.33,33.33]);
  ...
  await check('payment requires payer; debits once; purchase is not counted twice',async()=>{
    ...
    await db.query('SELECT pay_credit_card_bill($1,$2)',[bill,account]);
    assert.equal(Number(await scalar("SELECT count(*) FROM transactions WHERE flow='bill_payment'")),1);
    assert.equal(Number(await limit()),0);
    assert.equal(Number(await scalar("SELECT get_financial_overview()->>'saldo_disponivel'")),970);
    assert.equal(Number(await scalar("SELECT despesas FROM get_monthly_financial_summary('2026-01-01','2027-12-31')")),80);
  ...
  await check('deleting payment reopens bill and restores cash and outstanding limit',async()=>{
    ...
    assert.notEqual(await scalar('SELECT status FROM credit_card_bills WHERE id=$1',[bill]),'paga');
    assert.equal(Number(await limit()),80);
    assert.equal(Number(await scalar("SELECT get_financial_overview()->>'saldo_disponivel'")),1050);
  });
  await check('cross-owner references and malformed transfers are rejected',async()=>{
    ...
    await assert.rejects(()=>insertTx({type:'transfer',account_id:account,destination_account_id:foreignAccount}),/outro titular/);
    await assert.rejects(()=>insertTx({type:'transfer',account_id:account,destination_account_id:account}),/transactions_transfer_check/);
    await assert.rejects(()=>insertTx({type:'transfer',account_id:account,destination_account_id:wallet,credit_card_id:card}),/apenas contas/);
  ...
  await check('manual invoice cannot overwrite generated invoice or mark it paid',async()=>{
    await reject("INSERT INTO credit_card_bills(user_id,card_id,month,year,due_date,manual_amount) VALUES ($1,$2,9,2026,'2026-09-12',50)",[uid,card],/unique/);
  ...
  await check('shopping uses the same ledger and completion is idempotent',async()=>{
    const item=await scalar("INSERT INTO shopping_items(user_id,item,price,card_id,account_id,payment_method,installments,desired_date) VALUES ($1,'Planned',60,$2,$3,'credito_parcelado',2,'2026-09-06') RETURNING id",[uid,card,account]);
    ...
    assert.equal(Number(await scalar('SELECT count(*) FROM credit_card_bill_items WHERE transaction_id=$1',[id])),2);
    assert.equal(Number(await limit()),140);
    assert.equal(Number(await scalar("SELECT get_financial_overview()->>'saldo_disponivel'")),1050);
  });
  await check('cash history excludes purchases/transfers and includes payment only',async()=>{
    ...
    assert.equal(Number(extras.cash_series[0].net),0);
  });
  await check('viewer reads owner data but cannot mutate ledger or payment',async()=>{
    ...
    await assert.rejects(()=>insertTx({user_id:viewer,account_id:account}),/outro titular|row-level/);
  ...
  await check('cron can still generate non-card recurring transactions without JWT',async()=>{
    ...
    await db.query("INSERT INTO recurring_expenses(user_id,name,amount,account_id,billing_day,start_date) VALUES ($1,'Recurring',10,$2,1,'2026-01-01')",[uid,account]);
    await db.query('SELECT private.run_financial_daily_maintenance()');
    assert.equal(Number(await scalar('SELECT count(*) FROM transactions WHERE recurring_id IS NOT NULL')),1);
  });
