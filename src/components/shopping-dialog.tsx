...
export function ShoppingDialog({
  ...
  const entrada = useMemo(
    () => ({
      ...
      installments: Math.max(1, Math.round(parseNum(installments) || 1)),
      down_payment: parseNum(downPayment),
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display">
            {editing ? "Editar compra" : "Nova compra planejada"}
          </DialogTitle>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="rounded-xl border border-border/60 bg-secondary/30 p-4 space-y-2">
            ...
            <p className="text-xs">
              ...
              {payment === "credito_parcelado" && (
                <>
                  {" "}
                  · Parcela: <strong>{formatCurrency(analise.parcela)}</strong>
                </>
              )}
            </p>
            {analise.plano && (
              <p className="text-xs text-muted-foreground">
                Plano sugerido: guardar {formatCurrency(analise.plano.valorMensal)} por{" "}
                {analise.plano.meses} mês(es) — comprar com segurança a partir de{" "}
          ...
          <div className="flex items-center justify-between rounded-lg border border-border/60 p-3">
            <div>
              <Label htmlFor="sh-goal" className="text-sm">
                Criar meta de economia
              </Label>
              <p className="text-xs text-muted-foreground">
                Gera uma meta com o valor final da compra.
              </p>
            </div>
