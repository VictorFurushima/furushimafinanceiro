...
export function InvestmentDialog({
...
}: { open: boolean; onOpenChange: (o: boolean) => void; editing?: Investment | null }) {
  ...
  useEffect(() => {
    if (editing) {
      setName(editing.name); setType(editing.inv_type);
      ...
      setLiquidity(editing.liquidity); setRisk(editing.risk);
      setObjective(editing.objective ?? ""); setNotes(editing.notes ?? "");
      setStatus(editing.status); setReserve(editing.is_emergency_reserve);
    } else if (open) {
      setName(""); setType("cdb"); setInstitution(""); setInvested(""); setCurrent("");
      setInitial(""); setAppliedAt(toISODate(new Date())); setMaturity("");
      setLiquidity("diaria"); setRisk("baixo"); setObjective(""); setNotes("");
      setStatus("ativo"); setReserve(false);
    }
  }, [editing, open]);
  ...
  const submit = async (e: FormEvent) => {
    ...
    const parsed = schema.safeParse({
      name, inv_type: type, institution: institution.trim() || null,
      ...
      status, is_emergency_reserve: reserve,
    });
    ...
    try {
      ...
      if (editing) {
        ...
        await supabase.from("investment_events").insert({
          user_id: u.user.id, investment_id: editing.id, event_type: "alteracao",
          amount: 0, previous_amount: editing.current_amount, new_amount: parsed.data.current_amount,
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="inv-name" value={name} onChange={(e) => setName(e.target.value)}
              placeholder="Ex: CDB Banco X 110% CDI" required maxLength={120} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input id="inv-inst" value={institution} onChange={(e) => setInstitution(e.target.value)}
                placeholder="Ex: Nubank, XP..." maxLength={120} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="space-y-2">
              <Label htmlFor="inv-initial">Aplicado inicialmente (R$)</Label>
              <Input id="inv-initial" value={initial} onChange={(e) => setInitial(e.target.value)} placeholder="0,00" inputMode="decimal" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="inv-invested">Valor investido (R$)</Label>
              <Input id="inv-invested" value={invested} onChange={(e) => setInvested(e.target.value)} placeholder="0,00" inputMode="decimal" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="inv-current">Valor atual (R$)</Label>
              <Input id="inv-current" value={current} onChange={(e) => setCurrent(e.target.value)} placeholder="0,00" inputMode="decimal" />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label htmlFor="inv-applied">Data de aplicação</Label>
              <Input id="inv-applied" type="date" value={appliedAt} onChange={(e) => setAppliedAt(e.target.value)} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="inv-maturity">Vencimento (opcional)</Label>
              <Input id="inv-maturity" type="date" value={maturity} onChange={(e) => setMaturity(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="space-y-2">
              ...
              <Select value={liquidity} onValueChange={setLiquidity}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {LIQUIDITY_OPTIONS.map((l) => <SelectItem key={l.value} value={l.value}>{l.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {INVESTMENT_STATUS.map((s) => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          <div className="space-y-2">
            ...
            <Input id="inv-obj" value={objective} onChange={(e) => setObjective(e.target.value)}
              placeholder="Ex: Reserva, aposentadoria, viagem" maxLength={200} />
          </div>
          <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/40">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Destaca este investimento como reserva</p>
            </div>
            <Switch checked={reserve} onCheckedChange={setReserve} aria-label="Reserva de emergência" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="inv-notes">Observações</Label>
            <Textarea id="inv-notes" value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={1000} rows={2} />
          </div>
