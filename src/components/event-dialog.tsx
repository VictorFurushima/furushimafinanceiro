...
import {
  EVENT_CATEGORIES, PRIORITIES, RECURRENCE_OPTIONS, REMINDER_OPTIONS, toLocalInput,
} from "@/lib/schedule-constants";
import { pushEventToGoogle } from "@/lib/calendar-sync.functions";

export function EventDialog({
  open, onOpenChange, editing, defaultStart,
}: {
  open: boolean;
  ...
  const submit = async (e: FormEvent) => {
    ...
    const res = editing
      ? await supabase.from("calendar_events").update(payload).eq("id", editing.id).select("id").maybeSingle()
    ...
    if (syncEnabled && eventId) {
      ...
      if (!result.synced) {
        if (result.reason === "not_configured") {
          ...
          toast.error("Falha ao enviar ao Google. O compromisso local está seguro; tente reenviar.");
        }
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display">{editing ? "Editar compromisso" : "Novo compromisso"}</DialogTitle>
          <DialogDescription>Os dados ficam salvos no Furushima; o Google é apenas espelho.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input id="ev-start" type="datetime-local" value={startsAt}
                onChange={(e) => setStartsAt(e.target.value)} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="ev-end">Fim</Label>
              <Input id="ev-end" type="datetime-local" value={endsAt}
          ...
          <div className="flex items-center justify-between rounded-lg border border-border/50 px-3 py-2">
            <Label htmlFor="ev-alert" className="cursor-pointer">Criar alerta</Label>
            <Switch id="ev-alert" checked={createAlert} onCheckedChange={setCreateAlert} />
          </div>

          <div className="flex items-center justify-between rounded-lg border border-border/50 px-3 py-2">
            <div>
              <Label htmlFor="ev-sync" className="cursor-pointer">Adicionar ao calendário</Label>
              <p className="text-xs text-muted-foreground">Envia para o Google Calendar quando conectado.</p>
            </div>
