...
export default defineTool({
  ...
  description:
    "Lista cartões de crédito do usuário com limite total, limite usado e limite disponível.",
  inputSchema: {},
  ...
  handler: async (_input, ctx) => {
    ...
    const { data, error } = await supabaseForUser(ctx)
      .from("credit_cards")
      .select(
        "id, name, bank, total_limit, used_limit, closing_day, due_day, status, color, created_at",
      )
      .order("created_at");
