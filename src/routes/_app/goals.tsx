      ...
      {goals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {goals.map((g) => {
            ...
            return (
              <Card key={g.id} className="bg-gradient-card border-border/50 shadow-card overflow-hidden">
                  ...
                  <div>
                    ...
                    {daysLeft !== null && (
                      <p className="text-xs text-muted-foreground mt-1">
                        {daysLeft > 0 ? `${daysLeft} dias restantes` : daysLeft === 0 ? "Vence hoje" : "Atrasada"}
                      </p>
                    )}
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => { setEditing(g); setOpen(true); }} className="p-2 text-muted-foreground hover:text-primary">
                      <Pencil className="h-4 w-4" />
                    </button>
                    <button onClick={() => remove(g.id)} className="p-2 text-muted-foreground hover:text-destructive">
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-end justify-between">
                    <span className="font-display text-2xl font-bold">{formatCurrency(Number(g.current_amount))}</span>
                    <span className="text-sm text-muted-foreground">/ {formatCurrency(Number(g.target_amount))}</span>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Input id={`add-${g.id}`} placeholder="Adicionar R$" inputMode="decimal" className="h-9" onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        const v = parseFloat((e.target as HTMLInputElement).value.replace(",", "."));
                        if (v) { updateAmount(g, v); (e.target as HTMLInputElement).value = ""; }
                      }
                    }} />
                  </div>
                </CardContent>
