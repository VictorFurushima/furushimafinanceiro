...
import { CalendarClock, CheckCircle2, Circle, Plus, Repeat, Bell } from "lucide-react";
...
export const Route = createFileRoute("/_app/today")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Compromissos, rotinas, tarefas e alertas do seu dia em uma única tela." },
    ],
  }),
});
...
function TodayPage() {
  ...
  const { data: occurrences = [] } = useRoutineOccurrences(dateISO, dateISO);
  const { data: tasks = [] } = useTasks({ status: "pendente", limit: 20 });
  ...
  const toggleRoutine = async (routineId: string, done: boolean) => {
    if (!isAdmin || !user) return toast.error(VIEWER_MESSAGE);
  ...
  return (
    <div className="p-3 sm:p-6 lg:p-10 space-y-4 sm:space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <SummaryCard label="Compromissos" value={String(events.length)} />
        <SummaryCard label="Rotinas do dia" value={`${doneIds.size}/${todayRoutines.length}`} />
        <SummaryCard label="Tarefas pendentes" value={String(tasks.length)} />
        <SummaryCard label="Tempo livre estimado" value={fmtDuration(freeMinutes)} />
      </div>
