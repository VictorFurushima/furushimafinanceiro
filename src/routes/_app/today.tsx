...
import {
  useEvents, useRoutines, useRoutineOccurrences, useTasks, useAlerts,
} from "@/hooks/use-schedule-data";
import {
  categoryColor, endOfDay, fmtDateLong, fmtDuration, fmtTime, localDateISO, startOfDay,
...
export const Route = createFileRoute("/_app/today")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Compromissos, rotinas, tarefas, alertas e finanças do seu dia em uma única tela." },
    ],
  }),
});
  ...
  return (
    <div className="p-3 sm:p-6 lg:p-10 space-y-4 sm:space-y-6 max-w-5xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <p className="text-xs sm:text-sm text-muted-foreground capitalize">{fmtDateLong(today)}</p>
        ...
        {isAdmin && (
          <div className="flex gap-2">
            ...
            <Button variant="outline" className="flex-1 sm:flex-none min-h-11" onClick={() => setTaskOpen(true)}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <SummaryCard label="Rotinas do dia" value={`${doneIds.size}/${todayRoutines.length}`}
          loading={loadingRoutines || loadingOcc} />
        <SummaryCard label="Tarefas pendentes" value={String(tasks.length)} loading={loadingTasks} />
        <SummaryCard label="Tempo livre estimado" value={fmtDuration(freeMinutes)} loading={loadingAgenda} />
      </div>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        ...
        <CardContent>
          {loadingOverview || loadingSnapshot ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <MiniStat label="Saldo disponível" value={formatCurrency(overview?.saldo_disponivel ?? 0)} />
              <MiniStat label="Gastos do mês" value={formatCurrency(overview?.gastos_reais_mes ?? 0)} />
              <MiniStat label="Faturas em aberto" value={formatCurrency(overview?.faturas_abertas ?? 0)} />
              <MiniStat label="Contas vencendo (5d)"
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        ...
        <CardContent className="space-y-2">
          {loadingEvents ? (
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">Nenhum compromisso hoje.</p>
          ) : (
            events.map((e) => (
        ...
        <Card className="bg-gradient-card border-border/50 shadow-card">
          ...
          <CardContent className="space-y-2">
            {loadingRoutines || loadingOcc ? (
              ...
              <p className="text-sm text-muted-foreground py-6 text-center">Nenhuma rotina para hoje.</p>
            ) : (
              todayRoutines.map((r) => {
                const done = doneIds.has(r.id);
                return (
                  <button key={r.id} type="button" onClick={() => toggleRoutine(r.id, done)}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm truncate ${done ? "line-through text-muted-foreground" : ""}`}>{r.name}</p>
                      <p className="text-xs text-muted-foreground">
        ...
        <Card className="bg-gradient-card border-border/50 shadow-card">
          ...
          <CardContent className="space-y-2">
            {loadingTasks ? (
              ...
              <p className="text-sm text-muted-foreground py-6 text-center">Nada pendente. Bom trabalho!</p>
            ) : (
              tasks.slice(0, 8).map((t) => (
                <div key={t.id} className="flex items-center gap-3 rounded-lg border border-border/40 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {t.due_at && (
                      <p className="text-xs text-muted-foreground">
                        Prazo {new Date(t.due_at).toLocaleString("pt-BR", {
      ...
      {alerts.length > 0 && (
        <Card className="bg-gradient-card border-border/50 shadow-card">
          ...
          <CardContent className="space-y-2">
            {alerts.map((a) => (
              <div key={a.id} className="flex items-center justify-between gap-3 rounded-lg border border-border/40 p-3">
