...
function SettingsPage() {
  ...
  const addCategory = async (e: FormEvent) => {
    ...
    const { error } = await supabase.from("categories").insert({
      user_id: userId, name: newCat.trim(), type: newCatType, color: newCatColor, icon: "circle",
    });
  ...
  const setLimit = async (e: FormEvent) => {
    ...
    const { error } = await supabase.from("category_limits").upsert({
      user_id: userId, category_id: limitCat, monthly_limit: v,
    ...
    <div className="p-4 sm:p-4 sm:p-6 lg:p-10 max-w-5xl mx-auto space-y-6">
        ...
        <CardContent className="space-y-4">
          {isAdmin && (
            <form onSubmit={addCategory} className="grid grid-cols-1 md:grid-cols-[1fr,140px,80px,auto] gap-3">
              ...
              <Input type="color" value={newCatColor} onChange={(e) => setNewCatColor(e.target.value)} className="h-10" aria-label="Cor da categoria" />
              <Button type="submit" className="bg-gradient-primary text-primary-foreground" aria-label="Adicionar categoria">
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        ...
        <CardContent className="space-y-4">
          ...
          <ul className="space-y-2">
            {limits.length === 0 && <li className="text-sm text-muted-foreground">Nenhum limite definido.</li>}
            {limits.map((l) => {
              ...
              return (
                <li key={l.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isAdmin && (
                      <button onClick={() => delLimit(l.id)} aria-label="Excluir limite" className="text-muted-foreground hover:text-destructive">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    )}
                  </div>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">Usadas na análise de viabilidade do carrinho</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={savePrefs} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <Input id="min_reserve" inputMode="decimal" disabled={!isAdmin} value={String(prefs.min_reserve)}
                  onChange={(e) => setPrefs({ ...prefs, min_reserve: Number(e.target.value.replace(",", ".")) || 0 })} />
              </div>
              <div className="space-y-1">
                <Label htmlFor="max_free">% máx. do saldo livre</Label>
                <Input id="max_free" inputMode="decimal" disabled={!isAdmin} value={String(prefs.max_free_balance_pct)}
                  onChange={(e) => setPrefs({ ...prefs, max_free_balance_pct: Number(e.target.value.replace(",", ".")) || 0 })} />
              </div>
              <div className="space-y-1">
                <Label htmlFor="max_inst">% máx. da renda em parcelas</Label>
                <Input id="max_inst" inputMode="decimal" disabled={!isAdmin} value={String(prefs.max_income_installment_pct)}
                  onChange={(e) => setPrefs({ ...prefs, max_income_installment_pct: Number(e.target.value.replace(",", ".")) || 0 })} />
              </div>
            </div>
            ...
            <div className="rounded-lg bg-secondary/30 p-3 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Switch id="reminder" disabled={!isAdmin} checked={prefs.reminder_enabled}
              ...
              {prefs.reminder_enabled && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input id="rday" type="number" min={1} max={28} disabled={!isAdmin} value={prefs.reminder_day}
                      onChange={(e) => setPrefs({ ...prefs, reminder_day: Number(e.target.value) || 1 })} />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="ramount">Valor sugerido (R$)</Label>
                    <Input id="ramount" inputMode="decimal" disabled={!isAdmin} value={String(prefs.reminder_amount)}
                      onChange={(e) => setPrefs({ ...prefs, reminder_amount: Number(e.target.value.replace(",", ".")) || 0 })} />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="rmsg">Mensagem</Label>
                    <Input id="rmsg" disabled={!isAdmin} value={prefs.reminder_message ?? ""}
                      onChange={(e) => setPrefs({ ...prefs, reminder_message: e.target.value })} />
                  </div>
                  <div className="space-y-1">
                    <Label>Investimento recomendado</Label>
                    <Select value={prefs.reminder_investment_id ?? "none"} disabled={!isAdmin}
                      onValueChange={(v) => setPrefs({ ...prefs, reminder_investment_id: v === "none" ? null : v })}>
                      <SelectTrigger><SelectValue placeholder="Nenhum" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none">Nenhum</SelectItem>
                        {investments.filter((i) => i.status !== "resgatado").map((i) => (
            ...
            {isAdmin && (
              <Button type="submit" className="w-full sm:w-auto bg-gradient-primary text-primary-foreground">
                Salvar preferências
              </Button>
            )}
          </form>
        ...
        <Card className="bg-gradient-card border-border/50 shadow-card">
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Contas com acesso somente leitura aos seus dados. A pessoa precisa já ter uma conta criada.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <form onSubmit={grantViewer} className="grid grid-cols-1 sm:grid-cols-[1fr,auto] gap-3">
              <Input type="email" value={viewerEmail} onChange={(e) => setViewerEmail(e.target.value)}
                placeholder="email@exemplo.com" aria-label="E-mail do espectador" />
              <Button type="submit" className="bg-gradient-primary text-primary-foreground">
                <UserPlus className="h-4 w-4 mr-2" /> Conceder acesso
              </Button>
            </form>
            <ul className="space-y-2">
              {viewers.length === 0 && <li className="text-sm text-muted-foreground">Nenhum espectador cadastrado.</li>}
              {viewers.map((v) => (
                <li key={v.user_id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30">
                  <span className="flex items-center gap-2 text-sm">
                    <Eye className="h-4 w-4 text-muted-foreground" /> {v.email}
                  </span>
                  <Button variant="ghost" size="sm" onClick={() => revokeViewer(v.user_id)}
                    aria-label={`Revogar acesso de ${v.email}`} className="text-muted-foreground hover:text-destructive">
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </li>
              ))}
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardHeader><CardTitle className="font-display">Preferências gerais</CardTitle></CardHeader>
      ...
      <CardContent className="space-y-3 text-sm">
        <div className="flex items-center justify-between gap-3 p-3 rounded-lg bg-secondary/30">
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
              Envio unidirecional: o Furushima é a fonte de verdade e espelha os compromissos no Google.
            </p>
          </div>
          <span className={`text-xs shrink-0 ${connected ? "text-primary-glow" : "text-muted-foreground"}`}>
