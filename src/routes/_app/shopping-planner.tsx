...
import {
  Plus, Pencil, Trash2, ShoppingCart, CheckCircle2, CalendarClock,
  ExternalLink, Wallet, TrendingDown, Target,
} from "lucide-react";
...
import { analisarViabilidadeCompra, compararFormasPagamento, calcularPrecoFinal } from "@/lib/shopping-analysis";
import {
  SHOPPING_STATUS, SHOPPING_PRIORITIES, priorityLabel, priorityColor,
  purchaseTypeLabel, purchaseTypeColor, shoppingPaymentLabel,
  shoppingStatusLabel, shoppingStatusColor,
...
export const Route = createFileRoute("/_app/shopping-planner")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Planeje compras e veja se cabem no seu orçamento antes de gastar." },
    ],
  }),
});
...
function ShoppingPlannerPage() {
  ...
  const analisados = useMemo(
    () => items.map((i) => ({
      ...
      analise: analisarViabilidadeCompra(
        {
          price: i.price, shipping: i.shipping, discount: i.discount, interest: i.interest,
          priority: i.priority, purchase_type: i.purchase_type, payment_method: i.payment_method,
          installments: i.installments, down_payment: i.down_payment,
          category_id: i.category_id, desired_date: i.desired_date,
        },
        ctx.contexto,
      ),
    })),
  ...
  const pendentes = analisados.filter(({ item }) => !["comprado", "cancelado"].includes(item.status));
  ...
  const arriscadas = pendentes.filter(({ analise }) => analise.faixa === "risco" || analise.faixa === "inviavel").length;
  ...
  return (
    <div className="p-3 sm:p-6 lg:p-10 space-y-4 sm:space-y-6 max-w-6xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <p className="text-xs sm:text-sm text-muted-foreground">Decida antes de gastar</p>
          <h1 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold mt-1">Planejador de Compras</h1>
        </div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        <StatCard label="Saldo livre hoje" value={formatCurrency(ctx.contexto.saldoDisponivel - ctx.contexto.reservaMinima)} icon={Wallet} gradient />
        <StatCard label="Total planejado" value={formatCurrency(totalPlanejado)} icon={ShoppingCart} />
        <StatCard label="Compras viáveis" value={String(viaveis)} icon={CheckCircle2} accent="success" />
        <StatCard label="Compras arriscadas" value={String(arriscadas)} icon={TrendingDown} accent="destructive" />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filtered.map(({ item: i, analise }) => {
            ...
            const faixaColor =
              analise.faixa === "viavel" ? "#5FA498"
              : analise.faixa === "atencao" ? "#fbbf24"
              : analise.faixa === "risco" ? "#f97316" : "#f87171";
              ...
              <Card key={i.id} className="bg-gradient-card border-border/50 shadow-card">
                <CardHeader className="pb-3">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <CardTitle className="font-display text-base sm:text-lg leading-tight flex items-center gap-2 flex-wrap">
                        {i.item}
                        {i.link && (
                          <a href={i.link} target="_blank" rel="noopener noreferrer" aria-label={`Abrir link de ${i.item}`}>
                      ...
                      <div className="flex items-center gap-2 flex-wrap mt-1">
                        <Badge variant="outline" className="text-[10px]"
                          style={{ borderColor: shoppingStatusColor(i.status), color: shoppingStatusColor(i.status) }}>
                        ...
                        <Badge variant="outline" className="text-[10px]"
                          style={{ borderColor: purchaseTypeColor(i.purchase_type), color: purchaseTypeColor(i.purchase_type) }}>
                    ...
                    <div className="text-right">
                      <p className="font-display text-lg font-bold">{formatCurrency(calcularPrecoFinal(i))}</p>
                  ...
                  <div className="h-2 rounded-full bg-secondary/60 overflow-hidden mt-2">
                    <div className="h-full rounded-full transition-all" style={{ width: `${analise.score}%`, background: faixaColor }} />
                  </div>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button size="sm" variant="outline" onClick={() => setExpanded(isOpen ? null : i.id)}>
                    ...
                    {isAdmin && (
                      <>
                        <Button size="icon" variant="ghost" onClick={() => { setEditing(i); setOpen(true); }} aria-label="Editar compra">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => remove(i)} aria-label="Excluir compra">
                  ...
                  {isOpen && (
                    <div className="space-y-4 pt-2 border-t border-border/50">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                        <div><p className="text-muted-foreground">Impacto imediato</p><p className="font-medium">{formatCurrency(analise.impactoAtual)}</p></div>
                        <div><p className="text-muted-foreground">Impacto mensal</p><p className="font-medium">{formatCurrency(analise.impactoMensal)}</p></div>
                        <div><p className="text-muted-foreground">Saldo livre hoje</p><p className="font-medium">{formatCurrency(analise.saldoLivreAtual)}</p></div>
                        <div><p className="text-muted-foreground">Saldo livre mensal</p><p className="font-medium">{formatCurrency(analise.saldoLivreMensal)}</p></div>
                      </div>
                      ...
                      {analise.positivos.length > 0 && (
                        <div>
                          ...
                          <ul className="text-xs text-muted-foreground list-disc pl-4 space-y-0.5">
                            {analise.positivos.map((p, idx) => <li key={idx}>{p}</li>)}
                          </ul>
                        </div>
                      )}
                      {analise.negativos.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-destructive mb-1">Pontos de atenção</p>
                          <ul className="text-xs text-muted-foreground list-disc pl-4 space-y-0.5">
                            {analise.negativos.map((p, idx) => <li key={idx}>{p}</li>)}
                          </ul>
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <ul className="text-xs text-muted-foreground space-y-0.5">
                            {analise.metasAfetadas.map((m) => (
                              <li key={m.id}>{m.name}: atraso estimado de {m.atrasoMeses} mês(es)</li>
                            ))}
                          </ul>
                        </div>
                      )}
                      ...
                      {analise.plano && (
                        <div className="rounded-lg border border-border/60 bg-secondary/30 p-3 text-xs space-y-1">
                          ...
                          <p className="text-muted-foreground">
                            ...
                            {formatCurrency(analise.plano.valorMensal)} por mês, em {analise.plano.meses} mês(es) você
                            compra com tranquilidade — a partir de{" "}
                            {new Date(analise.plano.dataSegura).toLocaleDateString("pt-BR")}.
                          </p>
                        </div>
                      )}

                      <div>
                        <p className="text-xs font-medium mb-2">Comparação de formas de pagamento</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {comparacao.map((c) => (
                            <div key={c.metodo} className="rounded-lg border border-border/60 p-2.5 text-xs space-y-1">
                              <div className="flex items-center justify-between">
                                <span className="font-medium">{c.label}</span>
                                <Badge variant="outline" className="text-[10px]">risco {c.risco}</Badge>
                              </div>
                              <p className="text-muted-foreground">
                                Hoje: {formatCurrency(c.impactoImediato)} · Futuro: {formatCurrency(c.impactoFuturo)}
                              </p>
