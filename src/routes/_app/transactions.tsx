...
function TransactionsPage() {
  ...
  const onFilterChange = <T,>(setter: (v: T) => void) => (v: T) => { setPage(0); setter(v); };

  const clearFilters = () => {
    setPage(0);
    setSearch(""); setFType("all"); setFCat("all"); setFPay("all");
  ...
  const exportCSV = async () => {
    let q = supabase
      .from("transactions")
      .select("occurred_at, type, amount, description, payment_method, categories(name), accounts(name)")
  ...
  const pageTotal = rows.reduce((s, t) => s + (t.type === "income" ? Number(t.amount) : -Number(t.amount)), 0);
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {totalCount} no filtro · Saldo desta página: <span className={pageTotal >= 0 ? "text-success" : "text-destructive"}>{formatCurrency(pageTotal)}</span>
          </p>
        </div>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardContent className="p-4 space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <Input placeholder="Buscar..." value={search} onChange={(e) => onFilterChange(setSearch)(e.target.value)} />
            <Select value={fType} onValueChange={(v) => onFilterChange(setFType)(v as typeof fType)}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            <Input type="date" value={fFrom} onChange={(e) => onFilterChange(setFFrom)(e.target.value)} placeholder="De" />
            <Input type="date" value={fTo} onChange={(e) => onFilterChange(setFTo)(e.target.value)} placeholder="Até" />
            <Input type="number" placeholder="Valor mín" value={fMin} onChange={(e) => onFilterChange(setFMin)(e.target.value)} />
            <Input type="number" placeholder="Valor máx" value={fMax} onChange={(e) => onFilterChange(setFMax)(e.target.value)} />
