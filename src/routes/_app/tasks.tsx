...
import {
  categoryLabel, EVENT_CATEGORIES, PRIORITIES, priorityLabel, TASK_STATUSES, taskStatusLabel,
...
export const Route = createFileRoute("/_app/tasks")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Liste, priorize e conclua suas tarefas com prazos e categorias." },
    ],
  }),
});
              ...
              <Card key={t.id} className="bg-gradient-card border-border/50 shadow-card">
                <CardContent className="flex items-center gap-3 p-3">
                  <button type="button" onClick={() => toggle(t)}
                    ...
                    {done ? <CheckCircle2 className="h-5 w-5 text-primary-glow" /> : <Circle className="h-5 w-5" />}
                  </button>
                  ...
                  {isAdmin && (
                    <div className="flex gap-1 shrink-0">
                      <Button size="icon" variant="ghost" onClick={() => { setEditing(t); setOpen(true); }} aria-label="Editar tarefa">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => remove(t)} aria-label="Excluir tarefa">
