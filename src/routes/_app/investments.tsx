...
import {
  Plus, Pencil, Trash2, TrendingUp, TrendingDown, PiggyBank, Wallet,
  Percent, ShieldCheck, ArrowDownToLine, ArrowUpFromLine, RefreshCw, Calculator, BellRing,
} from "lucide-react";
...
import {
  INVESTMENT_TYPES, investmentTypeLabel, investmentTypeColor, riskLabel, riskColor,
  liquidityLabel, investmentStatusLabel, investmentStatusColor, INVESTMENT_EVENT_LABELS,
...
export const Route = createFileRoute("/_app/investments")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acompanhe aportes, resgates, rendimento e composição da sua carteira." },
    ],
  }),
});
...
function InvestmentsPage() {
  ...
  const reserva = ativos.filter((i) => i.is_emergency_reserve).reduce((s, i) => s + i.current_amount, 0);
  ...
  const evolucao = useMemo(() => {
    const months: { label: string; key: string; aporte: number; resgate: number; rendimento: number; acumulado: number }[] = [];
    const now = new Date();
    for (let i = 11; i >= 0; i--) {
      ...
      months.push({
        ...
        aporte: 0, resgate: 0, rendimento: 0, acumulado: 0,
      });
    }
    events.forEach((e) => {
    ...
    let acc = 0;
    months.forEach((m) => { acc += m.aporte - m.resgate + m.rendimento; m.acumulado = acc; });
    return months;
  }, [events]);

  const investidoVsAtual = ativos
    .slice(0, 8)
    .map((i) => ({ name: i.name.slice(0, 14), investido: i.invested_amount, atual: i.current_amount }));

  const participacao = ativos
    .map((i) => ({ name: i.name, value: i.current_amount, color: investmentTypeColor(i.inv_type) }))
    .sort((a, b) => b.value - a.value);

  const filtered = typeFilter === "all" ? investments : investments.filter((i) => i.inv_type === typeFilter);
  ...
  useEffect(() => {
    ...
    if (typeof Notification !== "undefined" && Notification.permission === "granted") {
      new Notification("Furushima Financeiro", {
        body: settings.reminder_message || `Hora do seu aporte mensal de ${formatCurrency(settings.reminder_amount)}.`,
      });
    }
  }, [settings, isAdmin]);
  ...
  const openMove = (inv: Investment, kind: MoveKind) => {
    if (!isAdmin) return toast.error(VIEWER_MESSAGE);
    setMoveTarget(inv); setMoveKind(kind); setMoveOpen(true);
  };
    ...
    <div className="p-3 sm:p-4 sm:p-6 lg:p-10 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold mt-1">Investimentos</h1>
        </div>
        {isAdmin && (
        ...
        <Alert className="border-primary/50 bg-primary/10">
          ...
          <AlertDescription className="text-xs sm:text-sm flex flex-col sm:flex-row sm:items-center gap-2">
            <span className="flex-1">
              ...
              {settings?.reminder_amount ? ` · sugerido ${formatCurrency(settings.reminder_amount)}` : ""}
            ...
            <span className="flex gap-2">
              ...
              }}>Registrar aporte agora</Button>
              <Button size="sm" variant="outline" onClick={() => dismissReminder(false)}>Lembrar depois</Button>
              <Button size="sm" variant="ghost" onClick={() => dismissReminder(true)}>Marcar como feito</Button>
            </span>
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        <StatCard label="Total investido" value={formatCurrency(totalInvestido)} icon={Wallet} gradient />
        <StatCard label="Valor atual" value={formatCurrency(valorAtual)} icon={PiggyBank} accent="success" />
        <StatCard label="Rendimento total" value={formatCurrency(rendimento)} icon={TrendingUp}
          accent={rendimento >= 0 ? "success" : "destructive"} />
        <StatCard label="Rentabilidade" value={`${rentabilidade.toFixed(2)}%`} icon={Percent}
          accent={rentabilidade >= 0 ? "success" : "destructive"} />
        <StatCard label="Rendimento mensal est." value={formatCurrency(rendimentoMensalEstimado)} icon={TrendingUp} />
        <StatCard label="Maior rendimento" value={maior ? maior.name : "—"} icon={TrendingUp}
          hint={maior ? formatCurrency(maior.rend) : undefined} accent="success" />
        <StatCard label="Menor rendimento" value={menor ? menor.name : "—"} icon={TrendingDown}
          hint={menor ? formatCurrency(menor.rend) : undefined} accent="destructive" />
        <StatCard label="Reserva de emergência" value={formatCurrency(reserva)} icon={ShieldCheck} />
      </div>
      ...
      {byType.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
          <Card className="bg-gradient-card border-border/50 shadow-card">
            <CardHeader><CardTitle className="font-display text-base sm:text-lg">Distribuição por tipo</CardTitle></CardHeader>
            <CardContent>
          ...
          <Card className="bg-gradient-card border-border/50 shadow-card">
            <CardHeader><CardTitle className="font-display text-base sm:text-lg">Evolução do patrimônio</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={230}>
                <AreaChart data={evolucao}>
                  ...
                  <Tooltip formatter={(v: number) => formatCurrency(v)} />
                  <Area type="monotone" dataKey="acumulado" stroke="#228E9A" fill="#228E9A" fillOpacity={0.25} name="Acumulado" />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          <Card className="bg-gradient-card border-border/50 shadow-card">
            <CardHeader><CardTitle className="font-display text-base sm:text-lg">Aportes e rendimento por mês</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={230}>
                <BarChart data={evolucao}>
                  ...
                  <Bar dataKey="rendimento" fill="#5FA498" name="Rendimento" radius={[4, 4, 0, 0]} />
          ...
          <Card className="bg-gradient-card border-border/50 shadow-card">
            <CardHeader><CardTitle className="font-display text-base sm:text-lg">Investido x Atual</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={230}>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardHeader><CardTitle className="font-display text-base sm:text-lg">Seus investimentos</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                return (
                  <li key={i.id} className="py-3 flex flex-wrap items-center gap-3">
                    <div className="h-10 w-10 rounded-xl flex items-center justify-center shrink-0"
                      style={{ background: `${investmentTypeColor(i.inv_type)}25`, color: investmentTypeColor(i.inv_type) }}>
                    ...
                    <div className="flex-1 min-w-[150px]">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="outline" className="text-[10px]"
                          style={{ borderColor: investmentStatusColor(i.status), color: investmentStatusColor(i.status) }}>
                          {investmentStatusLabel(i.status)}
                        </Badge>
                        {i.is_emergency_reserve && (
                          <Badge variant="outline" className="text-[10px]">Reserva</Badge>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {investmentTypeLabel(i.inv_type)}
                        {i.institution ? ` · ${i.institution}` : ""} · {liquidityLabel(i.liquidity)} ·{" "}
                        <span style={{ color: riskColor(i.risk) }}>risco {riskLabel(i.risk).toLowerCase()}</span>
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${rend >= 0 ? "text-success" : "text-destructive"}`}>
                        {rend >= 0 ? "+" : ""}{formatCurrency(rend)} ({pct.toFixed(2)}%)
                      </p>
                    </div>
                    {isAdmin && (
                      <div className="flex items-center gap-1">
                        <Button size="icon" variant="ghost" onClick={() => openMove(i, "aporte")} aria-label="Registrar aporte" title="Aporte">
                          <ArrowDownToLine className="h-4 w-4 text-success" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => openMove(i, "resgate")} aria-label="Registrar resgate" title="Resgate">
                          <ArrowUpFromLine className="h-4 w-4 text-warning" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => openMove(i, "valor")} aria-label="Atualizar valor" title="Atualizar valor">
                          <RefreshCw className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => { setEditing(i); setOpen(true); }} aria-label="Editar investimento">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => remove(i)} aria-label="Excluir investimento">
      ...
      {participacao.length > 0 && (
        <Card className="bg-gradient-card border-border/50 shadow-card">
          <CardHeader><CardTitle className="font-display text-base sm:text-lg">Participação por investimento</CardTitle></CardHeader>
              ...
              return (
                <div key={p.name}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground">{pct.toFixed(1)}% · {formatCurrency(p.value)}</span>
                  </div>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardHeader><CardTitle className="font-display text-base sm:text-lg">Histórico</CardTitle></CardHeader>
        <CardContent>
          {events.length === 0 ? (
                ...
                return (
                  <li key={e.id} className="py-2 flex items-center justify-between gap-3 text-sm">
                    <div className="min-w-0">
                      <p className="truncate">{INVESTMENT_EVENT_LABELS[e.event_type] ?? e.event_type} · {inv?.name ?? "—"}</p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(e.occurred_at).toLocaleDateString("pt-BR")}
                        {e.notes ? ` · ${e.notes}` : ""}
                      </p>
                    </div>
                    <span className={e.event_type === "resgate" ? "text-destructive" : "text-success"}>
                      {e.event_type === "resgate" ? "-" : "+"}{formatCurrency(Math.abs(e.amount))}
                    </span>
                  </li>
                );
              })}
      ...
      <InvestmentMoveDialog open={moveOpen} onOpenChange={setMoveOpen} investment={moveTarget} kind={moveKind} />
    </div>
  );
}
      ...
      <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="font-display text-2xl font-bold">
            {selic.rate !== null ? `${selic.rate.toFixed(2)}% a.a.` : selic.loading ? "Carregando..." : "—"}
          </p>
...
function Simulator({ selicRate }: { selicRate: number | null }) {
  ...
  const serie = useMemo(() => {
    ...
    for (let m = 1; m <= n; m++) {
      ...
      aportado += pm;
      rows.push({ mes: `${m}`, valor: Math.round(saldo * 100) / 100, aportado: Math.round(aportado * 100) / 100 });
    }
    return rows;
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="space-y-2">
            ...
            <Input id="sim-initial" value={initial} onChange={(e) => setInitial(e.target.value)} inputMode="decimal" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="sim-monthly">Aporte mensal</Label>
            <Input id="sim-monthly" value={monthly} onChange={(e) => setMonthly(e.target.value)} inputMode="decimal" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="sim-months">Meses</Label>
            <Input id="sim-months" value={months} onChange={(e) => setMonths(e.target.value)} inputMode="numeric" />
          </div>
          <div className="space-y-2">
            <Label>Taxa</Label>
            <Select value={useSelicRate ? "selic" : "custom"} onValueChange={(v) => setUseSelicRate(v === "selic")}>
          ...
          <div className="space-y-2">
            ...
            <Input id="sim-rate" value={useSelicRate && selicRate !== null ? selicRate.toFixed(2) : customRate}
              onChange={(e) => setCustomRate(e.target.value)} disabled={useSelicRate && selicRate !== null} inputMode="decimal" />
          </div>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          <StatCard label="Valor final" value={formatCurrency(last?.valor ?? 0)} icon={PiggyBank} gradient />
          <StatCard label="Total aportado" value={formatCurrency(last?.aportado ?? 0)} icon={Wallet} />
          <StatCard label="Rendimento" value={formatCurrency(rendimentoTotal)} icon={TrendingUp} accent="success" />
          <StatCard label="Rendimento médio/mês" value={formatCurrency(rendMedio)} icon={Percent} accent="success" />
        </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={serie}>
            ...
            <Legend wrapperStyle={{ fontSize: 11 }} />
            <Area type="monotone" dataKey="aportado" stroke="#20656C" fill="#20656C" fillOpacity={0.2} name="Aportado" />
            <Area type="monotone" dataKey="valor" stroke="#5FA498" fill="#5FA498" fillOpacity={0.25} name="Com rendimento" />
          </AreaChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
