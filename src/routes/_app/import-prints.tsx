  ...
  const { data: detected = [] } = useQuery({
    ...
    queryFn: async (): Promise<DetectedTx[]> => {
      ...
      return (data as DetectedTx[]).map((d) => ({
        ...d,
        detected_amount: d.detected_amount !== null ? Number(d.detected_amount) : null,
      }));
    },
  });
    ...
    <div className="p-3 sm:p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Envie prints de extratos, Pix, faturas ou comprovantes. O sistema lê automaticamente as
          transações.
        </p>
      </header>
                ...
                <div className="flex flex-wrap items-center gap-2 justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {img.ocr_confidence && (
                      <Badge
                        variant="outline"
                        className={cn("text-[10px]", confColor(img.ocr_confidence))}
                      >
...
function DetectedRow({
  ...
  const save = async () => {
    ...
    if (
      ...
      !confirm("Essa transação parece já ter sido registrada. Salvar mesmo assim?")
    )
      return;
    setSaving(true);
    try {
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {tx.confidence_level === "baixa" && (
          <span className="text-[11px] text-warning">
            Revise manualmente — leitura com baixa confiança
          </span>
        )}
      </div>
        ...
        <div className="space-y-1">
          <Label className="text-xs">
            ...
            {tx.suggested_category && (
              <span className="text-primary">· sugerida: {tx.suggested_category}</span>
            )}
          </Label>
