...
function RecurringPage() {
  ...
  const edit = (it: RecurringExpense) => {
    setEditing(it);
    setOpen(true);
  };
  const create = () => {
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto space-y-6">
      ...
      {upcoming.length > 0 && (
        <Card className="bg-gradient-card border-border/50 shadow-card">
          <CardHeader>
            <CardTitle className="font-display text-lg">Próximas cobranças</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="bg-gradient-card border-border/50 shadow-card">
        <CardHeader>
          <CardTitle className="font-display">Todas as assinaturas</CardTitle>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
              ...
              {items.map((i) => (
                <li key={i.id} className="flex items-center gap-4 py-3 group">
                  <div
                    ...
                    style={{
                      background: `${i.categories?.color ?? "#22d3ee"}25`,
                      color: i.categories?.color ?? "#22d3ee",
                    }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {i.categories?.name ?? "—"} ·{" "}
                      {FREQUENCIES.find((f) => f.value === i.frequency)?.label} · dia{" "}
                      {i.billing_day} · {paymentLabel(i.payment_method)}
                    </p>
                  </div>
                  <span className="text-sm font-semibold text-destructive">
